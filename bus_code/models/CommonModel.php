...
// print_r($trip_id);
    ...
    public function trackMap($trip_id,$bus_id,$driver_id,$status,$chaperone_id,$client_user)
   	{
   		 $today_date = date('Y-m-d');

   		$this->db->select("trip.id as trip_id,trip.trip_id as tridID,trip.client_user_id, trip.bus_id, trip.driver_id, trip.chaperone_id, trip.trip_date, trip.trip_start, trip.trip_end, trip.status,trip.note,trip.created_at,trip.updated_at,chaperone.id as chaperone_id,chaperone.chaperone_name,bus.bus_number,client.client_name,driver.driver_name,trip.pickup_latitude,trip.pickup_longitude,trip.drop_address,trip.drop_latitude,trip.drop_longitude")
    		->from('trip')
    		->join('bus','bus.id = trip.bus_id','left')
    		 ->join('driver','driver.id = trip.driver_id','left')
    		 ->join('chaperone','chaperone.id = trip.chaperone_id','left')
...
//--------------API Model ----------------------------------
    function checkChaperoneTrip($chaperone_id)
   	{
   		$this->db->select("trip.id as trip_id,trip.trip_id as tridID,trip.client_user_id, trip.bus_id, trip.driver_id, trip.chaperone_id, trip.trip_date, trip.trip_start, trip.trip_end, trip.status,trip.note,trip.created_at,trip.updated_at,trip.pickup_address,trip.pickup_latitude,trip.pickup_longitude,trip.drop_address,trip.drop_latitude,trip.drop_longitude,chaperone.id as chaperone_id,chaperone.chaperone_name,bus.bus_number,client.client_name,client.logo_image as client_logo_image,driver.driver_name,driver.drive_mobile_number,client_user.username as client_user_name,trip.complete_status")
    		->from('trip')
    		->join('bus','bus.id = trip.bus_id','left')
    		 ->join('driver','driver.id = trip.driver_id','left')
    		 ->join('chaperone','chaperone.id = trip.chaperone_id','left')
