...
class CommonModel extends CI_Model
{
	...
	public function demo($condition,$table)
	{
		return $this->db->query("SELECT * FROM $table WHERE $condition ")->result_array();
	}
	//fetch all data api
   ...
   public function sendMail($data)
   {
   	// print_r($data);exit;
		  ...
		  'smtp_host' => '204.11.58.156',
		  'smtp_user' => 'info@myprocurements.com',
		  'smtp_pass' => 'hardwork@123',
		  'smtp_port' => 587,
		  'crlf' => "\r\n",
		  'mailtype' => "html",
		  'charset' => "iso-8859-1",
		...
		$this->email->from('info@small_bazar.com', 'Small Bazar');
	...
	public function cartJoin($id)
	{
		// echo $id;exit;
		if (!empty($id)) {
			$this->db->select('tbl_booking.*,country.country_name,states.state_name,cities.city_name')->from('tbl_booking');
			$this->db->join('country','country.id = tbl_booking.country','left');
		    $this->db->join('states','states.id = tbl_booking.state','left');
		    $this->db->join('cities','cities.id = tbl_booking.city','left');
			$this->db->where(array("tbl_booking.user_id" => $id));
			return $this->db->get()->result_array();
		}else{
			$this->db->select('tbl_booking.*,country.country_name,states.state_name,cities.city_name')->from('tbl_booking');
			$this->db->join('country','country.id = tbl_booking.country','left');
	...
	public function productData(){
		$this->db->select("product.*,admin.name,category.category_name,sub_category.sub_category_name")
            ->from('product'); 
