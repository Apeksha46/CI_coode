                    ...
                    <div class="card table-responsive">
                        <div class="card-body">
                            <div class="form-group">
                                <input class="form-control form-control-lg" type="text" id="totalframework"
                                    value="{{$allFramework->count()}} Framework Found" disabled>
                            </div>
                            <div class="row">
                                <div class="col-md-10">
                                    <form action="">
                                        <div class="input-group">
                                            ...
                                            <input type="search" class="form-control form-control-lg"
                                                placeholder="Type Framework here" name="search_keyword"
                                                id="searchKeyword">
                                        </div>
                                        <div class="row">
                                            <div class="col-3">
                                ...
                                <div class="col-md-2">
                                    <button type="button" class="btn btn-success button1" id="ApplyButton"
                                        onclick="getFilterFramework()">Apply</button>
                    ...
                    <div class="card table-responsive">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-3">
                                    <div class="form-group">
                                        <input type="text" class="form-control total-selected-framework"
                                            value="0 Framework selected" disabled style="height: 42px;">
                                ...
                                <div class="col-7" align="right">
                                    <a href="{{ route('framework.create') }}" type="button" class="btn ipfs-button"><i
                                            class="fa fa-plus"></i> Add Framework</a>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-12">
                    <div class="card table-responsive">
                        <div class="card-header">
                            <h3 class="card-title">Framework Details</h3>
                        </div>
                        <div class="card-body">

                            <table id="frameworkTable" class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        <th>
                                ...
                                <tbody>
                                    ...
                                    $i = 1;
                                    @endphp
                                    @forelse ($allFramework as $p)

                                    <tr>
                                        <td>
                                            <div class="form-check1">
                                                <input class="form-check-input framework-selected-Id table-td-list-SelectAll"
                                        ...
                                        <td>
                                            <div class="form-check1">
                                                <!-- <input class="form-check-input framework-selected-Id"
                                                    type="checkbox" value="{{$p->id}}" name="p_id[]"
                                                    onclick="checkBox();"> -->
                                        ...
                                        <td>
                                            <form action="{{ route('framework.destroy', $p) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')

                                                <button type="button" class="btn ipfs-button edit-framework"
                                                    data-id="{{ $p->id }}"
                                                ...
                                                <button type="submit" class="btn ipfs-danger" data-toggle="tooltip"
                                                    data-placement="top" title="Delete Framework"
                                                    onclick="return confirm('Are you sure want to delete this Framework!')"><i
...
<div class="modal fade" id="editFramework">
    <div class="modal-dialog">
        <form action="{{ route('framework.update') }}" method="post" data-parsley-validate="">
            @csrf
...
@section('scripts')
<script>
var table = $('#frameworkTable').DataTable({
    "ordering": false,
    "language": {
...
$('#select_all').click(function(event) { //on click 
    ...
    table.column(0).nodes().to$().each(function(index) {
        if (checked) {
            var selected_box = $(this).find('.framework-selected-Id').prop('checked', true);
            total++;
            $('.checkStatus').prop("disabled", false);
        } else {
            $(this).find('.framework-selected-Id').prop('checked', false);
            $('.checkStatus').prop("disabled", true);
        }
    });
    var totalSelectedListing = total + ' Framework selected';
    $('.total-selected-framework').val(totalSelectedListing)
...
function checkBox() {
    ...
    var countchecked = table
        ...
        .find('.framework-selected-Id:checked').length;
    ...
    var totalSelectedListing = countchecked + ' Framework selected';
    $('.total-selected-framework').val(totalSelectedListing)
...
function changeStatusListing(id) {
    swal({
        ...
        if (willDelete) {
            ...
            var rowcollection = table.$(".framework-selected-Id:checked", {
                "page": "all"
            });
            ...
            $.ajax({
                type: "post",
                url: "{{ route('changeFrameworkStatus') }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    'status': id,
                    'framework_Id': checkedValue
                },
...
$('body').on('click', '.edit-framework', function() {
    var id = $(this).data('id');
    ...
    $('#editFramework').modal('show');
});

function change_status(id, value, status) {
    if (confirm("Are you sure want " + value + " this framework")) {

        $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            url: "{{ route('framework.status') }}",
            type: "POST",
            data: {
                "id": id,
                "status": status,
...
function populateDataTable(data) {
    ...
    $("#frameworkTable").DataTable().clear();
    ...
    if (length != 0) {
        for (var i = 0; i <= length; i++) {
            ...
            $('#frameworkTable').dataTable().fnAddData([
                ...
                status,
                '<form action="' + url +
                '" method="POST"><input type="hidden" name="_token" value="{{csrf_token()}}" /><input type="hidden" name="_method" value="DELETE"><button type="button" class="btn ipfs-button edit-framework" data-id="' +
                cat.id + '"  data-name="' + cat.name + '"><i class="fa fa-edit"></i> </button> <button type="submit" class="btn ipfs-danger" data-toggle="tooltip" data-placement="top" title="Delete Framework" onclick="return confirm(\'Are you sure want to delete this Framework!\')"><i class="fa fa-trash"></i></button></form>'
            ]);
            num++;
        }
    } else {
